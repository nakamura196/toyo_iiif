    ...
    while loop_flg:
        ...
        if not os.path.exists(path):
            ...
            try:
                ...
                src = "http://124.33.215.236/research/atoinga/"+str(book).zfill(3)+"/page"+str(page)+"/x1.jpg"
                print(src)

                image = Image.open(urllib.request.urlopen(src))
                width, height = image.size

                thumb = "http://124.33.215.236/research/atoinga/"+str(book).zfill(3)+"/page"+str(page)+"/thumbnail.jpg"
            except:
